...
const props = defineProps<{ publicState: GamePublicState; privateState: GamePrivateState | null; participants: RoomParticipant[]; actorId: string; isHost: boolean; connected: boolean }>()
...
const nicknameByActor = computed(() => new Map([
  ...props.participants.map(participant => [participant.actorId, participant.nickname] as const),
  ...(props.publicState.finalScores ?? []).map(entry => [entry.actorId, entry.nickname] as const),
]))
const hintByActor = computed(() => new Map(props.publicState.hints.map(hint => [hint.playerId, hint.text])))
const hintHistory = computed(() => (props.publicState.hintStatuses ?? props.publicState.hints.map(hint => ({ playerId: hint.playerId, status: 'SUBMITTED' as const })))
  .map(status => ({
    ...status,
    nickname: nicknameByActor.value.get(status.playerId) ?? status.playerId,
    text: hintByActor.value.get(status.playerId),
  })))
const liarName = computed(() => props.publicState.liarId
  ? nicknameByActor.value.get(props.publicState.liarId) ?? props.publicState.liarId
  : undefined)
...
<template>
  <div class="liar-game" data-region="liar-game">
    ...
    <section v-if="hintHistory.length" class="panel hint-history" data-region="hint-history" aria-labelledby="hint-history-title">
      <p class="eyebrow">HINT HISTORY</p><h2 id="hint-history-title">공개 힌트</h2>
      <ol>
        <li v-for="(entry, index) in hintHistory" :key="`${entry.playerId}-${index}`">
          <strong>{{ entry.nickname }}:</strong> {{ entry.status === 'SKIPPED' ? '힌트 건너뜀' : entry.text }}
        </li>
      </ol>
    </section>
    ...
    <DiscussionPanel v-else-if="publicState.phase === 'DISCUSSING'" :disabled="discussionDisabled" :proposal-active="publicState.submittedPlayerIds.length > 0" :can-propose="isHost" @propose="send('DISCUSSION_END_PROPOSE')" @approve="send('DISCUSSION_END_VOTE', { agree: true })" />
    <VotePanel v-else-if="publicState.phase === 'VOTING' || publicState.phase === 'REVOTING'" :participants="participants" :actor-id="actorId" :disabled="voteDisabled" :revote="publicState.phase === 'REVOTING'" :candidate-ids="publicState.revoteCandidates" @submit="send(publicState.phase === 'REVOTING' ? 'REVOTE_SUBMIT' : 'VOTE_SUBMIT', { targetActorId: $event })" />
    ...
    <LiarResultPanel v-else-if="publicState.phase === 'ROUND_RESULT' || publicState.phase === 'GAME_RESULT'" :result="publicState.roundResult" :final="publicState.phase === 'GAME_RESULT'" :disabled="actionDisabled" :is-host="isHost" :liar-name="liarName" :answer="publicState.answer" :final-scores="publicState.finalScores" @return-to-waiting="send('RETURN_TO_WAITING')" />
    <section v-else class="panel"><p>역할을 확인하고 다음 단계를 기다려 주세요.</p></section>
  </div>
</template>

<style scoped>
.liar-game { display: grid; gap: 1rem; }.game-status { display: flex; align-items: end; justify-content: space-between; gap: 1rem; }.game-status h1 { margin: 0; font-size: clamp(1.45rem, 3vw, 2.2rem); }.game-status > div:last-child { text-align: right; }.game-status > div:last-child p { margin: 0; color: #5843cf; font-weight: 850; }.game-status time { color: #686b7a; font-size: .8rem; }.eyebrow { margin: 0 0 .25rem; color: #6652d9; font-size: .68rem; font-weight: 900; letter-spacing: .12em; }.panel { border: 1px solid #e8e7ef; border-radius: 1rem; background: white; padding: 1.15rem; }.hint-history h2 { margin: 0; font-size: 1.1rem; }.hint-history ol { display: grid; gap: .4rem; margin: .75rem 0 0; padding-left: 1.25rem; }.panel form { display: grid; gap: .55rem; margin-top: 1rem; }.panel label { font-size: .78rem; font-weight: 800; }.panel input { border: 1px solid #dcdce6; border-radius: .7rem; padding: .7rem; }.panel button { border: 0; border-radius: .7rem; background: #5b47d6; color: white; cursor: pointer; font-weight: 850; padding: .7rem; }.panel button:disabled, .panel input:disabled { cursor: not-allowed; opacity: .55; }@media (max-width: 767px) { .game-status { align-items: start; flex-direction: column; }.game-status > div:last-child { text-align: left; } }
</style>
