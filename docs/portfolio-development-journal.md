...
- 현재 단계: 공통 플랫폼 수직 슬라이스 구현 및 최종 검증
- 기록 시작일: 2026-08-23
...
| ADR-104 | 2026-08-23 | WebSocket 연결은 사용자별 다중 세션을 추적하고 마지막 세션 종료 뒤 30초 유예 퇴장 | 탭 하나의 종료가 참가자를 퇴장시키지 않으면서 모바일 네트워크 전환을 복구하기 위해 | 구현 완료 |
| ADR-105 | 2026-08-23 | 참가자 스냅샷에 최근 채팅 100개를 허용 필드만 포함 | 이벤트 누락·새로고침 뒤 대화 문맥을 복구하되 비밀번호와 내부 상태를 노출하지 않기 위해 | 구현 완료 |
...
- 서버는 `ActorPrincipal`의 actor ID별 활성 STOMP 세션 집합을 관리한다. 여러 탭·기기 중 하나만 종료되면 참가 상태를 유지한다.
- 마지막 세션이 종료되면 기본 30초의 퇴장 작업을 예약한다. `APP_DISCONNECT_GRACE`로 조정할 수 있으며 테스트는 수동 스케줄러로 시간을 기다리지 않고 검증한다.
- 같은 actor가 유예 시간 안에 다시 연결되면 예약 작업을 취소한다. 유예가 만료되면 별도 삭제 로직을 만들지 않고 기존 `RoomApplicationService.leave` 경계를 사용한다. 따라서 `PLAYER_LEFT`, 방장 위임, 로비 upsert·remove, 빈 방 제거가 수동 퇴장과 같은 순서로 실행된다.
- 정상 STOMP `DISCONNECT`와 전송 계층의 비정상 종료 이벤트를 같은 멱등 처리기로 수렴시킨다.

#### 보호방 재진입과 채팅 복구

- 방 코드로 방을 찾은 뒤 저장소의 방 잠금 안에서 현재 참가자 여부를 다시 확인한다. 이미 참가 중이면 비밀번호 검증과 Argon2 연산, 이벤트 발행 없이 현재 스냅샷을 반환한다. 신규 참가자만 기존 비밀번호·정원·방 상태 규칙을 적용받는다.
- 이 규칙으로 비밀번호 방을 만든 방장이 대기방으로 이동하거나 페이지를 새로고침할 때 비밀번호를 다시 입력하지 않아도 된다.
- 참가자 전용 방 스냅샷은 최근 채팅 최대 100개를 `messageId`, `actorId`, `nickname`, `body`, `sentAt` 필드로만 제공한다. 비밀번호 원문·해시와 요청 제한 내부 상태는 DTO에 존재하지 않는다.
- Vue는 최초 로드, sequence 누락 복구, WebSocket 재연결 때 스냅샷 채팅을 권위 있는 최근 목록으로 교체한다. 이후 도착한 이벤트는 `messageId`로 병합해 중복을 버리고 최대 100개만 유지한다.

#### 단일 인스턴스 남용 방지

- 게스트 세션 생성은 프레임워크가 정규화한 `HttpServletRequest.getRemoteAddr()`를 기준으로 제한한다. 주소는 시간 버킷을 포함한 HMAC-SHA-256 지문으로 즉시 변환하며 원본 IP를 저장하거나 로그에 남기지 않는다. 지문은 기본 1시간마다 회전한다.
- 방 비밀번호 검증은 `actor + room`과 `HMAC IP 지문 + room` 두 범위를 동시에 소모한다. 제한 검사를 Argon2보다 먼저 실행하고, 올바른 비밀번호가 확인되면 해당 actor·network 성공 경로의 예산을 해제해 정상 사용자의 후속 입장을 방해하지 않는다.
- 기본값은 게스트 생성 분당 12회, 방 비밀번호 검증 분당 5회다. `APP_GUEST_RATE_CAPACITY`, `APP_GUEST_RATE_WINDOW`, `APP_PASSWORD_RATE_CAPACITY`, `APP_PASSWORD_RATE_WINDOW`으로 조정한다.
- 초과 응답은 HTTP 429, 안정 코드 `RATE_LIMITED`, 상관관계 `requestId`, 초 단위 `Retry-After` 헤더를 사용한다. 메모리 키는 원본 IP나 비밀번호를 포함하지 않고 최대 개수를 제한한다.

#### Cloud Run 운영 전제

- 현재 방 저장소, WebSocket presence, 연결 유예 예약, 남용 제한은 모두 프로세스 메모리이므로 운영 Cloud Run 서비스는 반드시 `max-instances=1`로 배포한다. 이 값은 성능 최적화가 아니라 데이터·보안 정합성 전제다.
...
- Vue 타입 검사와 독립적인 Playwright E2E 패키지는 TypeScript 7.0.2를 유지한다. 향후 `vue-tsc`가 TypeScript 7을 공식 지원하면 별도 업그레이드 PR에서 프론트 버전을 올리고 전체 타입 검사와 빌드를 다시 검증한다.
